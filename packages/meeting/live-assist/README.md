...
Each stable question gets its own answer. The VAD still supplies acoustic endpoints, but a fragment ending in a connective such as “负责” or “如果” stays in a text accumulator until the next fragment completes it. Question marks, clear sentence endings, and explicit multiple-question markers release one or more questions. The heuristic does not make an extra LLM call, so triage still sees the same single stable question and answers are generated one at a time in the order they were heard.
...
- Answers are serialized, so a burst of questions makes the later answers late. The accumulator only joins fragments matching its continuation and boundary rules; unusual speech may still be split or held until the next stable fragment.
