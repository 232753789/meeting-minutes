...
composer 工具行承载工具抽屉：每个 `conversation.input.tool` 条目在一个会话内渲染两次——一次为 `surface: 'bar'`（工具行中的图标，以及条目自己的对话框），一次为 `surface: 'drawer'`（一行 `ChoiceRow`，写出工具名称和一句说明它做什么的话）；抽屉的箭头按钮在工具行上方展开或收起面板。每个条目的 open 标志由抽屉而非条目持有，因此图标与抽屉行打开的是同一个对话框，点击抽屉行会关闭挡在其上的面板；同一时刻只有一个条目处于打开状态。运行中的工具把状态与控件留在比面板存活更久的 `bar` 面上；面板外的指针按下只收起面板，不影响已打开的工具。没有任何条目注册时，整个抽屉不渲染任何内容。`conversation.input.left` 仍是那些自带交互外观、应当紧挨常驻 chrome 的小型控件的座位。
